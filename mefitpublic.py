...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 19 22:23:53 2020
...
report = open(f"daily_report.txt", 'a')
report.write("Daily report for " + fdate)
...
'''
def route():
    global check, current_date
    rootdir = os.getcwd()
    #looking through all the files in the local directory 
    for subdir, dirs, files in os.walk(rootdir):
        for file in files:
            filepath = subdir + os.sep + file
            name = file.split(".csv")[0]
            #grabbing the file with list of locations 
            if name == "Locations":
            ...
            if name == "Min-Max_Price_Check":
                ...
                print(locs)
                check += 1
            #grabbing the daily transaction information 
...
def read_data(data1, data2, location,rows, check_name,data4,rows2,locs):
                        ...
                        if item[0] in locs:
                            for j in range(0, rows2):
                                if str(data4.iat[j, 0]).rstrip() == item[0] and (float(data1.iat[i, 2]) < float(data4.iat[j, 1]) or float(data1.iat[i, 2]) > float(data4.iat[j, 2])):
                                    with open("price_range_error.txt", 'a') as error_file:
    ...
    if count < rows:
        count += 1
        #opens a text file and writes the dict data to a formatted report
        report = open(f"daily_report.txt", 'a')
        report.write(location + '\n')
        #writes machine no. --> slot no. --> tuple with various values
        ...
        new_report = open(f"pubd_report.txt", 'a')
